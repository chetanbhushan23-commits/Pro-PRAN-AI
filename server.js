...
/* =========================================================
   AI PROVIDERS
   Keys are NEVER stored in GitHub. They must be supplied by
   the runtime environment (.env locally / Codespaces secret).
========================================================= */
    ...
    for (const command of commands) {
        ...
        if (!result.error && result.stdout.trim()) {
            try {
                ...
                attempts.push({ command, parse_error: parseError.message });
            }
        }
    }
...
function buildFallbackReport(symbol, quantData, verified, score, sentimentData, sources, researchQuality) {
    ...
    const newsLines = (sentimentData?.articles || []).slice(0, 15).map((a, n) =>
...
function buildAIPrompt(symbol, fullContext) {
    return `You are an evidence-first Indian Stock Market Quant Research Analyst.

Analyze ONLY the supplied JSON for ${symbol}.
Never invent, estimate, or add outside facts. null means N/A.
Never claim Dhan.
Never contradict verifiedTechnicalFacts.
Never manufacture OI/F&O, FII/DII, sector, support/resistance, targets, earnings, macro or regulatory claims unless supplied.
...
Treat TIER_1 publishers as higher-quality than DISCOVERY results, but do not call discovery articles false merely because of their tier.
...
Use researchQuality as evidence quality, not as a return forecast.
...
Quant Score
Final Recommendation
Numbered Source Traceability

Explicitly mention missing data and evidence limitations. Keep source URLs intact.
...
async function generateAIReport(prompt) {
    ...
    if (geminiModel) {
        try {
            console.log(`🤖 Gemini ACTIVE (${GEMINI_MODEL})`);
            return { text: await generateWithGemini(prompt), provider: "GEMINI", errors };
        } catch (error) {
    ...
    if (groq) {
        try {
            console.log(`⚡ Groq ACTIVE (${GROQ_MODEL})`);
            return { text: await generateWithGroq(prompt), provider: "GROQ", errors };
        } catch (error) {
...
app.get("/api/analyze", async (req, res) => {
    ...
    try {
        ...
        let sentimentData;
        try {
            sentimentData = await analyzeSentiment(symbol);
        } catch (error) {
            sentimentData = {
                sentiment: "N/A",
                score: null,
...
app.get("/api/research-quality", async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            sentimentData = { articles: [], lookback_days: 7, error: error.message };
        }
...
app.get("/api/ai-status", (req, res) => {
    res.json({ success: true, ...providerStatus() });
});

app.get("/api/ai-test", async (req, res) => {
    const prompt = "Reply with exactly: AI_PROVIDER_TEST_OK";
